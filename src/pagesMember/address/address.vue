...
const onchangeAddress = (address: AddressItem) => {
      ...
      <view v-if="addressList.length" class="address">
        <uni-swipe-action class="address-list">
          ...
          <uni-swipe-action-item class="item" v-for="address in addressList" :key="address.id">
            <view class="item-content" @tap="onchangeAddress(address)">
              <view class="user">
                {{ address.receiver }}
                <text class="contact">{{ address.contact }}</text>
                <text v-if="address.isDefault === 1" class="badge">默认</text>
              </view>
              <view class="locate">{{ address.fullLocation }}</view>
              <navigator
                class="edit"
                hover-class="none"
            ...
            <template #right>
              <button @tap="onDelAddress(address.id)" class="delete-button">删除</button>
            </template>
